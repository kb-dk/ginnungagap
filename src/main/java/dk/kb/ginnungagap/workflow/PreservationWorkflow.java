...
public class PreservationWorkflow extends AbstractWorkflow {
    ...
    protected void initialiseSteps() {
        ...
        for(String catalogName : server.getCatalogNames()) {
            steps.add(new PreserveSubAssetStep(conf, server, transformer, representationTransformer, preserver, catalogName));
            steps.add(new PreserveMasterAssetStep(conf, server, transformer, representationTransformer, preserver, catalogName));
            steps.add(new PreserveAllStep(conf, server, transformer, representationTransformer, preserver, catalogName));
        }

...
"${JAVA}" ${JAVA_OPTS} -D${assembly.home.env.name}="${assembly.home.env.name.ref}" -D${assembly.conf.env.file}="${assembly.conf.env.file.ref}" -cp "$CP" dk.kb.ginnungagap.CatalogStructmap "$@"
